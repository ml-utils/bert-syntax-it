...
def get_gpt_sentence_score(model, tokenizer: GPT2Tokenizer, tokenized_input, device):
    ...
    print(f'tokenizer.bos_token: {tokenizer.bos_token}, tokenizer.bos_token: {tokenizer.unk_token}')
    tensor_input = torch.tensor([[tokenizer.bos_token_id] + tokenizer.convert_tokens_to_ids(tokenized_input)], device=device)

    labels = torch.tensor([[tokenizer.bos_token_id] + tokenizer.convert_tokens_to_ids(tokenized_input)], device=device)
    labels[:, :1] = -1
...
def main():
    ...
    sentence_good_base = 'Gianni ha detto che il libro di linguistia ha duecento pagine.'
    sentence_bad = "Di che cosa Gianni ha detto che il libro ha duecento pagine?"
    score_sentence_good_base = __get_gpt_sentence_score(model, tokenizer, sentence_good_base)
    score_sentence_bad = __get_gpt_sentence_score(model, tokenizer, sentence_bad)
    print(f'score_sentence_good_base: {score_sentence_good_base}, score_sentence_bad: {score_sentence_bad}')
...
def __get_gpt_sentence_score(model, tokenizer, text, verbose=False):
    ...
    if verbose:
        print(f'type(output.loss) {type(output.loss)}, output.loss.size(): {output.loss.size()}')
        print(f'output[0].size() {output[0].size()}, output[1].size() {output[1].size()}, len(output[2]) {len(output[2])}')
