...
class Generator(data_generator.BenchmarkGenerator):
    def __init__(self):
        ...
        self.clause_embedding_adjectives = get_all(
            "category_2", "Adj_clausal", get_all("arg_1", "expression=it")
        )
    ...
    def sample(self):
        ...
        data = {
            "sentence_good": "%s %s %s it to be %s that %s."
            % (m_subj[0], Aux_raise[0], V_raise[0], Adj[0], sentence),
            "sentence_bad": "%s %s %s it to be %s that %s."
            % (m_subj[0], Aux_control[0], V_control[0], Adj[0], sentence),
            "two_prefix_prefix_good": "%s %s %s it to be"
            % (m_subj[0], Aux_raise[0], V_raise[0]),
            "two_prefix_prefix_bad": "%s %s %s it to be"
            % (m_subj[0], Aux_control[0], V_control[0]),
