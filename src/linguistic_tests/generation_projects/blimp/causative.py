...
class CSCGenerator(data_generator.BenchmarkGenerator):
    def __init__(self):
        ...
        self.alternating_verbs = np.union1d(
            get_all("causative", "1"), get_all("inchoative", "1")
        )
        self.non_alternating_intransitives = get_all(
            "causative", "0", all_intransitive_verbs
        )
    ...
    def sample(self):
        ...
        if V_cause["category"] == "S\\NP":
            ...
            if V_cause["3sg"] == "1":
                Subj = N_to_DP_mutate(
                    choice(np.intersect1d(all_animate_nouns, self.all_singulars))
                )
            elif V_cause["pres"] == "1":
                Subj = N_to_DP_mutate(
                    choice(np.intersect1d(all_animate_nouns, self.all_plurals))
                )
            else:
                Subj = N_to_DP_mutate(choice(all_animate_nouns))
        else:
            Subj = N_to_DP_mutate(
                choice(get_matches_of(V_cause, "arg_1", all_nominals))
            )
        ...
        if Subj["sg"] == "1":
            safe_verbs = np.intersect1d(
                self.non_alternating_intransitives, all_possibly_singular_verbs
            )
        else:
            safe_verbs = np.intersect1d(
                self.non_alternating_intransitives, all_possibly_plural_verbs
            )
        V_intrans = choice(
        ...
        data = {
            "sentence_good": "{} {} {} {}.".format(Subj[0], Aux[0], V_cause[0], Obj[0]),
            "sentence_bad": "{} {} {} {}.".format(
