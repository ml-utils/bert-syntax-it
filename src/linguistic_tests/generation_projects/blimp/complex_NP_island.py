...
class CSCGenerator(data_generator.BenchmarkGenerator):
    def __init__(self):
        ...
        self.predicates = np.union1d(
            all_refl_preds, get_all("category_2", "Adj_comp_than")
        )
    ...
    def sample(self):
        ...
        Subj = N_to_DP_mutate(
            choice(
                get_matches_of(
                    V_mat, "arg_2", get_matches_of(V_mat, "arg_1", self.safe_subjs)
                )
            )
        )
        ...
        Obj = N_to_DP_mutate(
            choice(
                get_matches_of(
                    ...
                    "arg_2",
                    get_matches_of(
                        V_mat, "arg_1", get_matches_of(Aux_mat, "arg_1", all_nominals)
                    ),
                )
            )
        )
        Wh = choice(
            get_matches_of(V_mat, "arg_2", get_matches_of(V_emb, "arg_2", all_wh_words))
        )
