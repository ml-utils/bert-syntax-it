...
class BindingGenerator(data_generator.BenchmarkGenerator):
    def __init__(self):
        ...
        self.special_verbs = np.append(
            get_all("expression", "imagine"),
            np.append(
                get_all("expression", "forget about"),
                get_all("expression", "think about"),
            ),
        )
    ...
    def sample(self):
        ...
        Vembed_finite = choice(
            get_matched_by(N1, "arg_1", get_all("finite", "1", Verb_embed))
        )
        ...
        data = {
            ...
            % (N1[0], V1[0], refl_match[0], Vembed_ing[0], N2[0]),
            "sentence_bad": "%s %s %s %s %s."
            % (N1[0], V1[0], refl_match[0], Vembed_finite[0], N2[0]),
            "one_prefix_prefix": "{} {} {}".format(N1[0], V1[0], refl_match[0]),
