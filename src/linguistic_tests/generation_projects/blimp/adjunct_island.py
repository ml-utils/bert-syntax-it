...
class CSCGenerator(data_generator.BenchmarkGenerator):
    ...
    def sample(self):
        ...
        V_emb = choice(
            get_matched_by(
                Obj, "arg_2", get_matched_by(Subj, "arg_1", self.all_ing_transitives)
            )
        )
        ...
        data = {
            ...
            % (Wh[0], Aux_mat[0], Subj[0], V_mat[0], Adv, V_emb[0], Obj[0]),
            "sentence_bad": "%s %s %s %s %s %s %s?"
            % (Wh[0], Aux_mat[0], Subj[0], V_mat[0], Obj[0], Adv, V_emb[0]),
        }
