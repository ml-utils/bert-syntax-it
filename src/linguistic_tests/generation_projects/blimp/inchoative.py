...
class Generator(data_generator.BenchmarkGenerator):
    def __init__(self):
        ...
        self.non_alternating_transitives = get_all(
            "inchoative", "0", all_transitive_verbs
        )
    ...
    def sample(self):
        ...
        if V_inch["category"] == "(S\\NP)/NP":
            if V_inch["3sg"] == "1":
                Subj = N_to_DP_mutate(
                    choice(get_matches_of(V_inch, "arg_2", self.all_singulars))
                )
            elif V_inch["pres"] == "1":
                Subj = N_to_DP_mutate(
                    choice(get_matches_of(V_inch, "arg_2", self.all_plurals))
                )
            else:
                Subj = N_to_DP_mutate(
                    choice(get_matches_of(V_inch, "arg_2", all_nominals))
                )
        else:
        ...
        if Subj["sg"] == "1":
            safe_verbs = np.intersect1d(
                self.non_alternating_transitives, all_possibly_singular_verbs
            )
        else:
            safe_verbs = np.intersect1d(
                self.non_alternating_transitives, all_possibly_plural_verbs
            )
        V_trans = choice(
        ...
        data = {
            "sentence_good": "{} {} {}.".format(Subj[0], Aux[0], V_inch[0]),
