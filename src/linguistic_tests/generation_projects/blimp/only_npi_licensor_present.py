...
class Generator(data_generator.BenchmarkGenerator):
    ...
    def sample(self):
        ...
        while reduce(
            lambda x, y: x or y,
            [args["subj"]["expression"].startswith(x) for x in bad_quantifiers],
        ):
            args = verb_args_from_verb(V, allow_negated=False)
        VP = V_to_VP_mutate(V, aux=False, args=args)

        data = {
            "sentence_good": "Only %s %s ever %s."
            % (args["subj"][0], args["aux"][0], VP[0]),
            "sentence_bad": "Even %s %s ever %s."
            % (args["subj"][0], args["aux"][0], VP[0]),
            "two_prefix_prefix_good": "Only {} {}".format(
                args["subj"][0], args["aux"][0]
            ),
            "two_prefix_prefix_bad": "Even {} {}".format(
                args["subj"][0], args["aux"][0]
            ),
