...
from src.linguistic_tests.utils.vocab_sets import all_transitive_verbs
from src.linguistic_tests.utils.vocab_sets import all_verbs
from src.linguistic_tests.utils.vocab_table import get_all
from src.linguistic_tests.utils.vocab_table import get_all_conjunctive
...
class AgreementGenerator(data_generator.BenchmarkGenerator):
    def __init__(self):
        ...
        ambiguous_verbs = list(
            filter(
                lambda verb: len(
                    list(
                        filter(
                            lambda x: x["root"] == verb["root"]
                            and x["past"] == "1"
    ...
    def sample(self):
        ...
        if random.choice([True, False]):
            ...
            V1_agree = choice(np.intersect1d(self.safe_verbs, all_transitive_verbs))
        N1 = N_to_DP_mutate(
            choice(get_matches_of(V1_agree, "arg_1", self.all_reg_nouns))
        )
        ...
        data = {
            "sentence_good": "{} {} {} {}.".format(N1[0], aux_agree, V1_agree[0], args),
            "sentence_bad": "%s %s %s %s."
            % (N1[0], aux_nonagree, V1_nonagree[0], args),
