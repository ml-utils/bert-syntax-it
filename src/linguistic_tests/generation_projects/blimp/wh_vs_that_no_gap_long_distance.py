...
class FillerGapGenerator(data_generator.BenchmarkGenerator):
    ...
    def sample(self):
        ...
        N3 = N_to_DP_mutate(
            choice(get_matches_of(V2, "arg_2", all_nouns))
        )  # needed to choose the correct wh-word, but N3 never gets used
        ...
        data = {
            "sentence_good": "%s %s that %s that %s %s %s %s."
            % (N1[0], V1[0], N2[0], V_rel[0], N_rel[0], V2[0], N3[0]),
            "sentence_bad": "%s %s %s %s that %s %s %s %s."
            % (N1[0], V1[0], wh[0], N2[0], V_rel[0], N_rel[0], V2[0], N3[0]),
        }
