...
class BindingGenerator(data_generator.BenchmarkGenerator):
    def __init__(self):
        ...
        self.all_safe_gendered_nouns = np.setdiff1d(
            self.all_gendered_singular_nouns, all_relational_nouns
        )
        self.all_sing_embedding_verbs = np.union1d(
            get_all_conjunctive([("pres", "1"), ("3sg", "1")], all_embedding_verbs),
            get_all("bare", "1", all_embedding_verbs),
        )
        self.all_sing_refl_preds = np.union1d(
            get_all_conjunctive([("pres", "1"), ("3sg", "1")], all_refl_preds),
            get_all("bare", "1", all_refl_preds),
        )
    ...
    def sample(self):
        ...
        N1 = N_to_DP_mutate(
            choice(get_matches_of(V1, "arg_1", self.all_safe_gendered_nouns))
        )
        ...
        data = {
            "sentence_good": "%s %s %s %s %s."
            % (N1[0], V1[0], N2[0], Vembed[0], refl_match[0]),
            "sentence_bad": "%s %s %s %s %s."
            % (N2[0], V1[0], N1[0], Vembed[0], refl_match[0]),
            "two_prefix_prefix_good": "{} {} {} {}".format(
                N1[0], V1[0], N2[0], Vembed[0]
            ),
            "two_prefix_prefix_bad": "{} {} {} {}".format(
                N2[0], V1[0], N1[0], Vembed[0]
            ),
