...
from src.linguistic_tests.utils.string_utils import string_beautify
...
class AgreementGenerator(data_generator.BenchmarkGenerator):
    def __init__(self):
        ...
        self.all_inanim_subj_allowing_verbs = get_matched_by(
            choice(all_inanimate_nouns), "arg_1", all_transitive_verbs
        )
        self.all_anim_subj_allowing_verbs = get_matched_by(
            choice(all_animate_nouns), "arg_1", all_transitive_verbs
        )
        self.all_anim_subj_verbs = np.setdiff1d(
            self.all_anim_subj_allowing_verbs, self.all_inanim_subj_allowing_verbs
        )
        self.dets = ["the", "some"]
        self.location_nouns = get_all("locale", "1")
        self.nonlocation_commonnouns = np.setdiff1d(
            all_common_nouns, self.location_nouns
        )
    ...
    def sample(self):
        ...
        data = {
            "sentence_good": "%s %s %s by %s %s."
            % (N1[0], cop[0], V1[0], det, N2_good[0]),
            "sentence_bad": "%s %s %s by %s %s."
            % (N1[0], cop[0], V1[0], det, N2_bad[0]),
            "one_prefix_prefix": "{} {} {} by {}".format(N1[0], cop[0], V1[0], det),
