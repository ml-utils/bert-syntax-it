...
class Generator(data_generator.BenchmarkGenerator):
    def __init__(self):
        ...
        self.good_quantifiers_sg = reduce(
            np.union1d,
            [
                get_all("expression", s, all_determiners)
                for s in good_quantifiers_sg_str
            ],
        )
        self.good_quantifiers_pl = reduce(
            np.union1d,
            [
                get_all("expression", s, all_determiners)
                for s in good_quantifiers_pl_str
            ],
        )
        bad_emb_subjs = reduce(
            np.union1d,
            (all_relational_poss_nouns, all_proper_names, get_all("category", "NP")),
        )
    ...
    def sample(self):
        ...
        while no_match:
            try:
                ...
                V_control = choice(
                    np.intersect1d(get_same_aux_verbs(V_raise), self.control_verbs)
                )
                m_subj = N_to_DP_mutate(
                    choice(
                        get_matches_of(
                            V_raise, "arg_1", get_matches_of(V_control, "arg_1")
                        )
                    )
                )
            except Exception:
        ...
        D = (
            choice(get_matched_by(emb_subj, "arg_1", self.good_quantifiers_sg))
            if emb_subj["sg"] == "1"
        ...
        data = {
            "sentence_good": "%s %s %s there to be %s %s %s."
            % (m_subj[0], Aux[0], V_raise[0], D[0], emb_subj[0], VP[0]),
            "sentence_bad": "%s %s %s there to be %s %s %s."
            % (m_subj[0], Aux[0], V_control[0], D[0], emb_subj[0], VP[0]),
            "two_prefix_prefix_good": "{} {} {}".format(m_subj[0], Aux[0], V_raise[0]),
            "two_prefix_prefix_bad": "{} {} {}".format(m_subj[0], Aux[0], V_control[0]),
