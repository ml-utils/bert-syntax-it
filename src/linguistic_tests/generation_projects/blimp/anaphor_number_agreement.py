...
class AnaphorGenerator(data_generator.BenchmarkGenerator):
    def __init__(self):
        ...
        self.all_safe_singular_nouns = np.setdiff1d(
            all_singular_nouns, all_singular_neuter_animate_nouns
        )
        self.all_safe_plural_nouns = np.setdiff1d(
            all_plural_nouns, all_singular_neuter_animate_nouns
        )

        self.all_singular_reflexive_predicates = np.setdiff1d(
            all_refl_preds, all_strictly_plural_transitive_verbs
        )
        self.all_plural_reflexive_predicates = np.setdiff1d(
            all_refl_preds, all_strictly_singular_transitive_verbs
        )
    ...
    def sample(self):
        ...
        if random.choice([True, False]):
            ...
            N1 = choice(
                get_matches_of(
                    V1, "arg_1", get_matches_of(V1, "arg_2", self.all_safe_plural_nouns)
                )
            )
            ...
            N1 = choice(
                get_matches_of(
                    V1,
                    "arg_1",
                    get_matches_of(V1, "arg_2", self.all_safe_singular_nouns),
                )
            )
