...
class SuperlativeGenerator(data_generator.BenchmarkGenerator):
    def __init__(self):
        ...
        self.singular_quantifiers = np.extract(
            ["sg=1" in x["arg_1"] and x["expression"] != "no" for x in all_determiners],
            all_determiners,
        )
    ...
    def sample(self):
        ...
        data = {
            ...
            % (Q[0], N1[0], V[0], Qsup, Num, N2[0]),
            "sentence_bad": "No {} {} {} {} {}.".format(N1[0], V[0], Qsup, Num, N2[0]),
            "two_prefix_prefix_good": "%s %s %s %s"
            % (Q[0], N1[0], V[0], Qsup.split()[0]),
            "two_prefix_prefix_bad": "No {} {} {}".format(N1[0], V[0], Qsup.split()[0]),
