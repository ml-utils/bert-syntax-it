...
class Generator(data_generator.BenchmarkGenerator):
    def __init__(self):
        ...
        good_quantifiers_pl_str = ["no", "some", "few", "many"]
        ...
        self.bad_quantifiers = reduce(
            np.union1d,
            [get_all("expression", s, all_determiners) for s in bad_quantifiers_str],
        )
        bad_subjs = reduce(
            np.union1d,
    ...
    def sample(self):
        ...
        d_good = (
            choice(get_matched_by(subj, "arg_1", self.good_quantifiers_sg))
            if subj["sg"] == "1"
        ...
        data = {
            "sentence_good": "There %s %s %s %s."
            % (args["aux"][0], d_good[0], subj[0], VP[0]),
            "sentence_bad": "There %s %s %s %s."
            % (args["aux"][0], d_bad[0], subj[0], VP[0]),
        }
