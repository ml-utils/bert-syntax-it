...
def conjugate(
    verb, subj, allow_negated=True, require_negated=False, change_v_form=False
):
    """
    ...
    if allow_negated:
        ...
        subj_agree_auxiliaries = get_matched_by(
            subj, "arg_1", all_non_negated_modals_auxs
        )
    if require_negated:
    ...
    if len(verb_agree_auxiliaries) == 0:
        print(
            f"no verb_agree_auxiliaries for verb {verb[0]} and len(subj_agree_auxiliaries) {len(subj_agree_auxiliaries)}"
        )
...
def re_conjugate(verb, subj, aux):
    ...
    if (
        verb["pres"] == "1"
        and subj["category_2"] == "nom_pronoun"
        and subj["sg"] == "1"
        and (subj["person"] == "1" or subj["person"] == "2")
    ):
        verb = get_all_conjunctive(
            [("root", verb["root"]), ("pres", "1"), ("3sg", "0")]
        )[0]
    else:
        verb = get_matches_of(
            aux,
            ...
            get_matched_by(
                subj,
                "arg_1",
                get_all_conjunctive([("root", verb["root"]), ("pres", verb["pres"])]),
            ),
        )[0]
...
def return_aux(verb, subj, allow_negated=True, require_negated=False, allow_modal=True):
    ...
    aux = choice(
        get_matched_by(verb, "arg_2", get_matched_by(subj, "arg_1", safe_auxs))
    )
    return aux
...
def require_aux(verb, subj, allow_negated=True, require_negated=False):
    ...
    if allow_negated:
        ...
        subj_agree_auxiliaries = get_matched_by(
            subj, "arg_1", all_non_negative_auxiliaries_no_null
        )

    if require_negated:
        subj_agree_auxiliaries = get_matched_by(
            subj, "arg_1", all_negative_auxiliaries_no_null
        )
    ...
    if verb["finite"] == "1":
        ...
        if allow_negated:
            if choice([True, False]):
                subj_agree_auxiliaries = get_matched_by(
                    subj, "arg_1", all_non_negative_agreeing_aux
                )
                subj_nonagree_auxiliaries = np.setdiff1d(
                    all_non_negative_agreeing_aux, subj_agree_auxiliaries
                )
            else:
                subj_agree_auxiliaries = get_matched_by(
                    subj, "arg_1", all_negative_agreeing_aux
                )
                subj_nonagree_auxiliaries = np.setdiff1d(
                    all_negative_agreeing_aux, subj_agree_auxiliaries
                )
        else:
            subj_agree_auxiliaries = get_matched_by(
                subj, "arg_1", all_non_negative_agreeing_aux
        ...
        verb_agree_auxiliaries = get_matched_by(verb, "arg_2", subj_agree_auxiliaries)
        verb_nonagree_auxiliaries = get_matched_by(
            verb, "arg_2", subj_nonagree_auxiliaries
        )
