...
class Generator:
    ...
    def make_logger(self, metadata):
        ...
        if not os.path.exists(log_dir):
            os.mkdir(log_dir)
        logging.basicConfig(
            filename=os.path.join(log_dir, log_file), level=logging.DEBUG
        )
    ...
    def generate_paradigm(
        self, number_to_generate=1, rel_output_path=None, absolute_path=None
    ):
        """
        ...
        print(
            "Generating data for "
            + constant_data["UID"]
            + f" (number_to_generate {number_to_generate})"
        )
    ...
    def generate_paradigm(
        ...
        while len(past_sentences) < number_to_generate:
            try:
                ...
                if error_counter >= number_to_generate // 10:
                    raise Exception(
                        r"Over 10\% of samples result in errors. You should fix this."
                    )

    def build_presupposition_paradigm(
        self,
        unembedded_trigger=None,
        negated_trigger=None,
        interrogative_trigger=None,
        modal_trigger=None,
        conditional_trigger=None,
        presupposition=None,
        negated_presupposition=None,
        neutral_presupposition=None,
    ):
        triggers = []
    ...
    def generate_paradigm(
        ...
        for file in ["test.jsonl", "train.jsonl"]:
            ...
            if not self.control_paradigm:
                ...
                output_control_writer = jsonlines.Writer(
                    output_control_file, flush=True
                )
            split_counter = 0
                ...
                try:
                    ...
                    if not overlap:
                        ...
                        if file == "test.jsonl":
                            ...
                            for line in list(
                                filter(lambda x: x["condition"] == "training", new_data)
                            ):
                                output_writer.write(line)
                        split_counter += 1
                        paradigmID += 1
