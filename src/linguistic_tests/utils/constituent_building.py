...
def verb_phrase_from_subj(subject, frequent=True, allow_negated=True):
    ...
    args = verb_args_from_verb(
        verb=verb, frequent=frequent, subj=subject, allow_negated=allow_negated
    )
    VP = V_to_VP_mutate(verb, frequent=frequent, args=args)
    return VP


def verb_args_from_verb(
    verb,
    frequent=True,
    subj=None,
    aux=None,
    ...
    if subj is None:
        subj = N_to_DP_mutate(
            choice(get_matches_of(verb, "arg_1", get_all("category", "N", freq_vocab))),
    ...
    if aux is None:
        args["aux"] = return_aux(
            verb, args["subj"], allow_negated=allow_negated, allow_modal=allow_modal
        )
    else:
        args["aux"] = aux
    ...
    if verb["category"] == "(S\\NP)/NP":
        args["args"] = [
            N_to_DP_mutate(
                choice(
                    get_matches_of(verb, "arg_2", get_all("category", "N", freq_vocab))
                ),
                allow_quantifiers=allow_quantifiers,
            )
        ]
    ...
    if verb["category"] == "(S\\NP)/(S[from]\\NP)":
        obj = N_to_DP_mutate(
            choice(get_matches_of(verb, "arg_2", freq_vocab)),
            allow_quantifiers=allow_quantifiers,
        )
        if allow_recursion:
            VP = V_to_VP_mutate(
                choice(get_matched_by(obj, "arg_1", all_ing_verbs)),
                frequent=frequent,
                aux=False,
            )
        else:
            ...
            VP = V_to_VP_mutate(
                choice(get_matched_by(obj, "arg_1", safe_verbs)),
                frequent=frequent,
                aux=False,
            )
        VP[0] = "from " + VP[0]
    ...
    if verb["category_2"] == "V_control_object":
        obj = N_to_DP_mutate(
            choice(get_matches_of(verb, "arg_2")), allow_quantifiers=allow_quantifiers
        )
        if allow_recursion:
...
def pred_args_from_pred(pred, frequent=True, subj=None, allow_negated=True):
    ...
    # all verbs have a subject
    if subj is None:
        args["subj"] = N_to_DP_mutate(
            choice(get_matches_of(pred, "arg_1", get_all("category", "N", freq_vocab)))
        )
    else:
        args["subj"] = subj
...
def make_sentence_from_verb(verb, frequent=True, allow_recursion=False):
    ...
    return " ".join(
        [args["subj"][0], args["aux"][0], verb[0]] + [x[0] for x in args["args"]]
    )
...
def V_to_VP_mutate(verb, aux=True, frequent=True, args=None, allow_recursion=False):
    ...
    if args is None:
        args = verb_args_from_verb(
            verb, frequent=frequent, allow_recursion=allow_recursion
        )
    if aux:
...
def make_sentence(frequent=True, allow_recursion=False):
    ...
    verb[0] = make_sentence_from_verb(
        verb, frequent=frequent, allow_recursion=allow_recursion
    )
    return verb
...
def make_sentence_from_args(args):
    ...
    """
    return " ".join(
        [args["subj"][0], args["aux"][0], args["verb"][0]]
...
def noun_args_from_noun(
    noun, frequent=True, allow_recursion=False, allow_quantifiers=True, avoid=None
):
    """
    ...
    if allow_quantifiers:
        matched = get_matched_by(
            noun, "arg_1", np.intersect1d(all_determiners, sample_space)
        )
        if len(matched) == 0:
            print(
                f"no matches for noun {noun[0]}, arg_1, all_determiners, sample_space[0] {sample_space[0]}"
            )
        else:
        ...
        matched = get_matched_by(
            noun,
            "arg_1",
            get_all("quantifier", "0", np.intersect1d(all_determiners, sample_space)),
        )
        if len(matched) == 0:
            print(
                f"no matches for noun {noun[0]}, arg_1, quantifier, 0, all_determiners {all_determiners}, sample_space {sample_space}"
            )
        args["det"] = choice(matched)
    if noun["category"] == "N":
        args["args"] = []
    ...
    if noun["category"] == "N/NP":
        if allow_recursion:
            obj = N_to_DP_mutate(
                choice(
                    get_matches_of(
                        noun, "arg_1", np.intersect1d(all_nominals, sample_space)
                    )
                )
            )
        else:
            obj = N_to_DP_mutate(
                choice(
                    get_matches_of(
                        noun, "arg_1", np.intersect1d(all_nouns, sample_space)
                    )
                )
            )
        args["args"] = [obj]
...
def N_to_DP_mutate(
    noun, frequent=True, determiner=True, allow_quantifiers=True, avoid=None
):
    """
    ...
    """
    args = noun_args_from_noun(
        noun, frequent, allow_quantifiers=allow_quantifiers, avoid=avoid
    )
    if determiner and args["det"] is not []:
        noun[0] = " ".join([args["det"][0], noun[0]] + [x[0] for x in args["args"]])
    else:
        noun[0] = " ".join([noun[0]] + [x[0] for x in args["args"]])
    return noun
...
def subject_relative_clause(noun):
    ...
    VP[0] = remove_extra_whitespace("{} {}".format(rel[0], VP[0]))
...
def negate_VP(verb, aux):
    ...
    if aux["expression"] == "":
        aux_neg = (
            get_all("expression", "didn't")[0]
            if verb["past"] == "1"
            else get_all("expression", "doesn't")[0]
            if verb["3sg"] == "1"
...
def embed_V_args_under_modal(V_args):
    ...
    """
    aux_under_modal, verb_under_modal = get_VP_under_modal_form(
        V_args["aux"], V_args["verb"]
    )
...
def get_same_V_form(root, verb_to_match):
    ...
    matches = list(
        filter(
            lambda v: v["finite"] == verb_to_match["finite"]
            and v["bare"] == verb_to_match["bare"]
            and v["pres"] == verb_to_match["pres"]
            and v["past"] == verb_to_match["past"]
            and v["ing"] == verb_to_match["ing"]
            and v["en"] == verb_to_match["en"]
            and v["3sg"] == verb_to_match["3sg"],
            verbs,
        )
    )
    if len(matches) == 0:
        raise LexicalGapError(
            "No verb with root {} matching form of {}".format(root, str(verb_to_match))
        )
    elif len(matches) == 1:
        return matches[0]
    else:
        raise NonUniqueError(
            "More than one verb with root %s matching form of %s"
            % (root, str(verb_to_match))
        )
...
def build_locative(locale, allow_quantifiers=True, avoid=None, bind_det=False):
    if bind_det:
        ...
        locale = N_to_DP_mutate(
            locale, allow_quantifiers=allow_quantifiers, avoid=avoid
        )
    if locale["locative_prepositions"] == "":
        raise FieldAbsentError(
            'Item %s is missing field "locative_prepositions".' % (locale[0])
        )
    locale[0] = (
        random.choice(locale["locative_prepositions"].split(";")) + " " + locale[0]
    )
    return locale
