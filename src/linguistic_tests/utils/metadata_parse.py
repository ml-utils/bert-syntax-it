...
def read_data_tsv(data_file_path):
    ...
    data_type = [(k, "U100") for k in keys] + [
        ("judgment", "i1"),
        ("sentence", "U10000"),
        ("original_metadata", "U10000"),
    ]
    data_table = []
    ...
    for in_domain in subsets:
        ...
        for e in in_domain:
            for line in list(filter(lambda x: x["env"] == e, stack)):
                ...
                writer.write(
                    "%s\t%s\t\t%s"
                    % (line["original_metadata"], line["judgment"], line["sentence"])
                )
                if writer == test:
...
def make_splits(test_size=1000, dev_size=1000, train_size=10000):
    ...
    for file in os.listdir(npi_path):
        if file[-4:] == ".tsv":
            ...
            read_file = np.union1d(
                get_all("npi", "ever", read_file), get_all("npi", "any", read_file)
            )
            dir_name = file[12:-4]
            ...
            for p in paradigms[
                test_iters + dev_iters : test_iters + dev_iters + train_iters
            ]:
                for line in list(filter(lambda x: x["paradigm"] == p, read_file)):
                    train.write(
                        "%s\t%s\t\t%s"
    ...
    for split_folder in split_folders:
        ...
        for metadata_label in metadata_labels:
            if not os.path.isdir(
                os.path.join(probing_path, split_folder, metadata_label)
            ):
            ...
            for file in split_folder_files:
                ...
                if metadata_label != "scope_with_licensor":
                    ...
                    lines = [
                        re.sub(
                            "\t[0-9]\t\t", "\t" + x.split("scope=")[1][0] + "\t\t", x
                        )
                        for x in lines
                        if len(x) != 0 and "licensor=1" in x
                    ]
                outfile = open(
                    os.path.join(probing_path, split_folder, metadata_label, file), "w"
                )
                for line in lines:
                    outfile.write(line + "\n")
                infile.close()
            ...
            test_full = open(
                os.path.join(
                    probing_path, split_folder, metadata_label, "test_full.tsv"
                ),
                "r",
            )
            test_full_sentences = test_full.readlines()
            outfile = open(
                os.path.join(probing_path, split_folder, metadata_label, "test.tsv"),
                "w",
            )
...
def make_combines():
    ...
    for folder in split_folders + ["all_env"]:
        ...
        test_full = open(
            os.path.join(npi_path, "combs", folder_tag, "test_full.tsv"), "w"
        )
